...
# select() helpers: functions that can be specifically used with select() to make life easier
## Goal: Output the columns with names that contain the letter a
select(parks_tb, contains("a"))

## Goal: Output the columns with names that start with "park"
select(parks_tb, starts_with("park"))

## Goal: Output the columns that contain numeric vectors
select(parks_tb, where(is.numeric))
...
# filter() helpers: functions that can be specifically used with filter() to make life easier
filter(parks_tb, between(year, 1980, 1999))
